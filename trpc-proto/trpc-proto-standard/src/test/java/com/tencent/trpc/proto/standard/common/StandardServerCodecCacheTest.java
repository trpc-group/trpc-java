...
public class StandardServerCodecCacheTest {
    ...
    public void testParseFuncWithLegalFunc() throws Exception {
        ...
        Assertions.assertEquals(SERVICE_NAME, funcInfo[0]);
        Assertions.assertEquals(METHOD_NAME, funcInfo[1]);
        // only the last separator is used to split the service and the method
        String[] multiLevel = parseFunc("/a/b/c");
        Assertions.assertEquals("a/b", multiLevel[0]);
        Assertions.assertEquals("c", multiLevel[1]);
    }
    ...
    public void testParseFuncWithIllegalFunc() throws Exception {
        ...
        for (String illegalFunc : illegalFuncs) {
            ...
            Assertions.assertEquals(StringUtils.EMPTY, funcInfo[0], illegalFunc);
            Assertions.assertEquals(StringUtils.EMPTY, funcInfo[1], illegalFunc);
        }
    }
    ...
    public void testGetOrComputeReusesCachedValue() throws Exception {
        ...
        Assertions.assertEquals(1, funcInfoCache.estimatedSize());
        Assertions.assertNotNull(funcInfoCache.getIfPresent(FUNC));
    }

    @Test
    public void testGetOrComputeCachesKeyOfMaxLength() throws Exception {
        String key = buildFunc(cacheKeyMaxLength);
        Assertions.assertEquals(cacheKeyMaxLength, key.length());
        String[] value = getOrCompute(funcInfoCache, key);
        Assertions.assertEquals(METHOD_NAME, value[1]);
        Assertions.assertEquals(1, funcInfoCache.estimatedSize());
        Assertions.assertSame(value, funcInfoCache.getIfPresent(key));
    }

    @Test
    public void testGetOrComputeNeverCachesOversizedKey() throws Exception {
        String key = buildFunc(cacheKeyMaxLength + 1);
        Assertions.assertEquals(cacheKeyMaxLength + 1, key.length());
        ...
        Assertions.assertEquals(METHOD_NAME, first[1]);
        Assertions.assertNotSame(first, second);
        Assertions.assertNull(funcInfoCache.getIfPresent(key));
        Assertions.assertEquals(0, funcInfoCache.estimatedSize());
    }
    ...
    public void testCacheIsBoundedBySize() throws Exception {
        ...
        funcInfoCache.cleanUp();
        Assertions.assertTrue(funcInfoCache.estimatedSize() <= cacheMaxSize,
                "cache size should be bounded, but was " + funcInfoCache.estimatedSize());
    }
    ...
    public void testHotKeyIsKeptWhileCacheIsFlooded() throws Exception {
        ...
        funcInfoCache.cleanUp();
        Assertions.assertTrue(funcInfoCache.estimatedSize() <= cacheMaxSize);
        Assertions.assertSame(hot, funcInfoCache.getIfPresent(FUNC));
    }

    @Test
    public void testDecodeParsesAndCachesFuncInfo() {
        Request request = decode(buildRequestHead(FUNC, CALLER, CALLEE));
        Assertions.assertEquals(FUNC, request.getInvocation().getFunc());
        Assertions.assertEquals(SERVICE_NAME, request.getInvocation().getRpcServiceName());
        Assertions.assertEquals(METHOD_NAME, request.getInvocation().getRpcMethodName());
        Assertions.assertNotNull(funcInfoCache.getIfPresent(FUNC));
        Assertions.assertEquals(1, funcInfoCache.estimatedSize());
        // the second decoding of the same func reuses the cached entry
        Request another = decode(buildRequestHead(FUNC, CALLER, CALLEE));
        Assertions.assertEquals(SERVICE_NAME, another.getInvocation().getRpcServiceName());
        Assertions.assertEquals(1, funcInfoCache.estimatedSize());
    }
    ...
    public void testDecodeParsesAndCachesCallInfo() {
        ...
        Assertions.assertEquals(1, callInfoCache.estimatedSize());
        // the same caller/callee/method reuses the cached entry
        Request another = decode(buildRequestHead(FUNC, CALLER, CALLEE));
        Assertions.assertSame(callInfo, another.getMeta().getCallInfo());
        Assertions.assertEquals(1, callInfoCache.estimatedSize());
    }
    ...
    public void testDecodeWithOversizedFuncDoesNotPolluteCache() {
        ...
        Request request = decode(buildRequestHead(func, CALLER, CALLEE));
        Assertions.assertEquals(METHOD_NAME, request.getInvocation().getRpcMethodName());
        Assertions.assertNull(funcInfoCache.getIfPresent(func));
        Assertions.assertEquals(0, funcInfoCache.estimatedSize());
    }
    ...
    public void testDecodeWithOversizedCallerDoesNotPolluteCache() {
        ...
        Request request = decode(buildRequestHead(FUNC, caller, CALLEE));
        Assertions.assertEquals(caller, request.getMeta().getCallInfo().getCaller());
        Assertions.assertEquals(0, callInfoCache.estimatedSize());
    }
    ...
    public void testDecodeWithBlankCallInfoIsNotCached() {
        ...
        Assertions.assertEquals(StringUtils.EMPTY, request.getInvocation().getRpcServiceName());
        Assertions.assertEquals(StringUtils.EMPTY, request.getInvocation().getRpcMethodName());
        Assertions.assertEquals(0, callInfoCache.estimatedSize());
        Assertions.assertNull(callInfoCache.getIfPresent(StringUtils.EMPTY));
    }
    ...
    public void testDecodeWithDistinctFuncKeepsCachesBounded() {
        ...
        funcInfoCache.cleanUp();
        callInfoCache.cleanUp();
        Assertions.assertTrue(funcInfoCache.estimatedSize() <= Math.min(total, cacheMaxSize));
        Assertions.assertTrue(callInfoCache.estimatedSize() <= Math.min(total, cacheMaxSize));
    }
