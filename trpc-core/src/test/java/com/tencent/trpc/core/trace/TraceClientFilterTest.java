...
public class TraceClientFilterTest {
    ...
    TracerClientFilter filter = new TracerClientFilter() {
        ...
        @Override
        public Span start(SpanBuilder spanBuilder, RpcContext context,
                Invoker<?> invoker, Request request) {
                ...
                for (Entry<String, String> entry : traceMap.entrySet()) {
                    if (entry.getValue() != null) {
                        attachments.put(entry.getKey(),
                                entry.getValue().getBytes(TracerConstants.DEFAULT_CHARSET));
                    }
                }
            } catch (Exception e) {
